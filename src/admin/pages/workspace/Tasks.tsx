...
export const WorkspaceTasks = ({ projectId, initialTasks, onUpdate }: WorkspaceTasksProps) => {
  ...
  const toggleTaskStatus = async (task: ProjectTask) => {
    ...
    await updateTask(task.id, { status: nextStatus[task.status] as any });
    onUpdate();
  };
    ...
    <div className="space-y-8">
      ...
      {!showAddForm ? (
        <button 
          ...
          className="w-full p-4 rounded-2xl border border-dashed border-white/10 hover:border-accent-violet/50 hover:bg-white/5 transition-all flex items-center justify-center gap-2 text-white/40 group"
        >
          <Plus className="w-5 h-5 group-hover:text-accent-violet" />
          <span className="text-sm font-medium group-hover:text-white">Create a New Task</span>
        </button>
      ) : (
        <motion.form 
