...
export const WorkspaceNotes = ({ projectId, initialNotes, onUpdate }: WorkspaceNotesProps) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      {/* Note Creation Area */}
      <div className={`p-6 rounded-3xl bg-white/5 border border-white/10 transition-all ${isAdding ? 'ring-1 ring-accent-violet/30 bg-white/7' : ''}`}>
          ...
          <AnimatePresence>
            {initialNotes.map((note) => (
              <motion.div
                ...
                className="group p-6 rounded-3xl bg-white/5 border border-white/10 hover:border-accent-violet/20 hover:bg-white/7 transition-all relative"
              >
