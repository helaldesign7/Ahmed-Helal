    ...
    <>
      <div className={`fixed bottom-10 ${isRtl ? 'left-10' : 'right-10'} z-50`}>
        ...
        >
          ...
          <div
            className={`absolute ${isRtl ? 'left-full ml-6' : 'right-full mr-6'} top-1/2 -translate-y-1/2 px-6 py-3 rounded-3xl bg-black/90 backdrop-blur-2xl border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-500 whitespace-nowrap pointer-events-none transform group-hover:translate-y-[-50%] shadow-2xl`}
          >
            <span
              className={`text-[10px] font-black tracking-[0.2em] uppercase text-accent-violet ${
            ...
            <div
              ...
              } w-3 h-3 bg-white/5 rotate-45 border-l border-b border-white/10 z-[-1] lg:block hidden`}
            />
          </div>
        </motion.button>
      </div>
